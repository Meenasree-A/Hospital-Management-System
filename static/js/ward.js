$(document).ready(function () {
    ...
    function addward(data) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("Ward Added Successfully", {"status":"success"});
            table.destroy();
    ...
    function deleteward(Ward_ID) {
        ...
        swal({
           ...
           $.ajax(settings).done(function (response) {
             swal("Deleted!", "Ward has been deleted.", "success");
             table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getward()
        });
    ...
    function updateward(data, Ward_ID) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "ward/" + Ward_ID,
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("Ward Updated Successfully", {"status":"success"});
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getward()
        });
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('.btn-edit').one("click", function(e) {
                ...
                $('#myModal').modal().one('shown.bs.modal', function (e) {
                    ...
                    $("#savetheward").off("click").on("click", function(e) {
                        var instance = $('#detailform').parsley();
                        instance.validate()
                        console.log(instance.isValid())
                        if(instance.isValid()){
                            jsondata = $('#detailform').serializeJSON();
                            updateward(jsondata, data.Ward_ID)
                        }
